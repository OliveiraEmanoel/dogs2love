...
class _DetailsDogState extends State<DetailsDog> {
    ...
    return Scaffold(
        ...
        body: Container(
            ...
            child: ListView.builder(
                itemCount: (_indexActual <= picturesUrl.length)
                    ? tempList.length + 1
                    : tempList.length, //picturesUrl.length + 1,
                      ...
                      ? Container(
                          ...
                          child: Center(
                            child: (tempList.length == 0)
                                ...
                                : (tempList.length == picturesUrl.length)
                                    ? Text('Final do arquivo',
                                        style: TextStyle(
                                          fontSize: 20,
                                        ))
                                    : FlatButton(
                                        child:
                                            Text("Clic aqui para mais fotos...",
                                                style: TextStyle(
                                                  fontSize: 20,
                                                )),
                                        onPressed: () {
                                          setState(() {
                                            if ((_indexActual + _itemsPerPage) >
                                                picturesUrl.length) {
                                              tempList.addAll(
                                                  picturesUrl.getRange(
                                                      _indexActual,
                                                      picturesUrl.length));
                                            } else {
                                              tempList.addAll(
  ...
  Future getBreedsPic(String breed) async {
    ...
    setState(() {
      //loading the first ten pictures
